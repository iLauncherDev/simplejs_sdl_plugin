...
simplejs_status_t sdl3_proxy_create_window(simplejs_function_header_t *function_header)
{
    ...
    sdl_window = SDL_CreateWindow(title_string,
                                  (int)simplejs_variable_get_int(width_var),
                                  (int)simplejs_variable_get_int(height_var),
                                  (SDL_WindowFlags)simplejs_variable_get_int(flags_var));
    if (!sdl_window)
    {
    ...
    simplejs_variable_assign(function_header->return_variable, &ret_var);
...
error:
    if (!SIMPLEJS_SUCCESS(status))
    {
        ...
        simplejs_variable_assign(function_header->return_variable, &undef_var);
    }

    return SIMPLEJS_STATUS_SUCCESS;
}

simplejs_status_t sdl3_proxy_create_renderer(simplejs_function_header_t *function_header)
{
    simplejs_status_t status = SIMPLEJS_STATUS_SUCCESS;
...
simplejs_status_t sdl3_proxy_destroy_renderer(simplejs_function_header_t *function_header)
{
...
simplejs_status_t sdl3_proxy_destroy_window(simplejs_function_header_t *function_header)
{
    if (function_header->argument_count < 1)
        goto error;
...
simplejs_status_t sdl3_proxy_create_event(simplejs_function_header_t *function_header)
{
    simplejs_status_t status = SIMPLEJS_STATUS_SUCCESS;
...
simplejs_status_t sdl3_proxy_poll_event(simplejs_function_header_t *function_header)
{
    simplejs_status_t status = SIMPLEJS_STATUS_SUCCESS;
    simplejs_object_t *object_event = NULL;
    if (function_header->argument_count < 1)
    ...
    simplejs_variable_init_number(&temp_in, &temp_num);
    simplejs_variable_assign(function_header->return_variable, &temp_in);
...
simplejs_status_t sdl3_proxy_set_render_draw_color(simplejs_function_header_t *function_header)
{
    ...
    Uint8 r = (Uint8)simplejs_variable_get_int(r_var);
    Uint8 g = (Uint8)simplejs_variable_get_int(g_var);
    Uint8 b = (Uint8)simplejs_variable_get_int(b_var);
    Uint8 a = (Uint8)simplejs_variable_get_int(a_var);
...
simplejs_status_t sdl3_proxy_render_clear(simplejs_function_header_t *function_header)
{
    if (function_header->argument_count < 1)
        goto error;
...
simplejs_status_t sdl3_proxy_render_present(simplejs_function_header_t *function_header)
{
    if (function_header->argument_count < 1)
        goto error;
...
simplejs_status_t sdl3_proxy_set_render_vsync(simplejs_function_header_t *function_header)
{
    if (function_header->argument_count < 2)
        goto error;
    ...
    SDL_SetRenderVSync(sdl_renderer, (int)simplejs_variable_get_int(mode_var));

error:
    return SIMPLEJS_STATUS_SUCCESS;
}

simplejs_status_t sdl3_proxy_get_ticks_ns(simplejs_function_header_t *function_header)
{
    ...
    simplejs_variable_assign(function_header->return_variable, &tmp_var);

    return SIMPLEJS_STATUS_SUCCESS;
}

simplejs_status_t sdl3_proxy_print(simplejs_function_header_t *function_header)
{
    char tempStringBuffer[4096];
    char *string = NULL;
